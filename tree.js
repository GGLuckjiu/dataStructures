...
function createByPreMid(pre,mid,i,j,count){
	...
	node.right = createByPreMid(pre,mid,i+1+len,j+1+len,count-1-len);
	return node;
}
function create(){
	let pre = 'abhfdeckg',mid = 'hbdfaekcg';
...
function searchParent(node,value){
	...
	if((node.left && node.left.value===value) || (node.right && node.right.value === value)){
		return node;
	}
